...
\[\mathrm{d}\phi \coloneqq f_{\Theta|X}\cdot\mathrm{d}\mu \]
is a \(\sigma\)-finite measure on the parameter space \(\Theta\). Obviously we can not normalise it in general since it does not need to be finite. However the ergodic theorem \ref{ergTheo} still holds for sigma finite measures and yields that the restrictions of \(\hat{\mathbb P}_n\) onto sets of finite measure \(\mu(A)<\infty\) converge weakly towards \(\mu\) up to normalisation. The argument for the stationarity of the \(\sigma\)-finite measure \(\pi\) stays exactly the same as before. 
\begin{figure}[h!]
	\centering
...
Hence, we can take a completely analogue approach to the approximation of the likelihood function, but we only present the result of the third and final MCMC simulation in Figure \ref{fig:MHrwwup}.
...
\subsubsection*{A naive approximation of posterior}

The motivation for the use of MCMC methods was that one wants to obtain an approximation of the posterior. We present here a different and naive approach, which works a lot faster and with higher accuracy. However we will see later that this approach suffers from what is known as the \emph{course of dimensionality}, i.e. the time needed to perform it will grow exponentially with the dimension of the parameter that should be estimated.
...
involves the computation of a \(N\times N\) matrix. This can be done explicitly using Gauss elimination which does not change the determinant -- at least up to a sign -- and can be performed in at most \(N^3\) steps\todo{is this true?}. Hence, the time neede for the computation of the likelihood function can be bounded -- up to a constant -- by
...
Also the results shown in the Figures \ref{fig:MHrw}, \ref{fig:MHrwwup} and \ref{fig:directHeatMaps} suggest that the approximation gained by this direct approach are more accurate\todo{find an argument for this}, not everything is perfect. In fact, if we denote the dimension of the parameter by \(M\), then the size of the grid of breakpoints needed for the interpolation grows exponentially in \(M\). If we denote the number of grid lines along each coordinate by \(R\), then one needs to evaluate the posterior \(R^M\) times and the times for this behaves like
\begin{equation}\label{complexityDirect}
O\left( R^M\cdot N^3\right).
\end{equation}
This exponential increase in necessary evaluations leads to the effect that this direct approach is only possible if the parameter one wishes to estimate is very low dimensional.\todo{How does \(T\) grow with \(M\)?}\todo{comment on complexity for silce sampling?}
...
\section{Stability analysis under noise}
