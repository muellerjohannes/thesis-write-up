...
We chose the sample size of \(n=8\) relatively small and want to show the effect the sample size has on the estimation. Obviously, we know from the second chapter that the MLE will converge to the actual parameter. To show this convergence in our case, we iteratively raised the sample size from \(1\) to \(20\) and obtained the maximum likelihood estimators that are shown in Figure \ref{fig:learningcurve}. We can see that the MLE converges towards the actual parameter where the regularised MLE seems to converge to a very similar value where the difference of the limits is smaller than the fluctuations. This is because the regularisation is not very strong in this case and we will see later that the models are actually quite close together meaning that the Bayes factor is not very high. \todo{check this} % \todo{does this mean the effect of the regularisation vanishes?}
\begin{figure}[h!]
	\centering
