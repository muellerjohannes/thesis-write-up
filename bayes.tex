...
where we used MarkovÕs inequality as well as \(y>0\) almost surely. Now draw \(x_{n+1}\) according to the uniform distribution on \(S\). Note that \(f(x_n)>0\), then \(f(x_{n+1})\ge y > 0\) almost surely.  In pseudo code for the algorithm can is presented in Algorithm \ref{alg:slice-sampling}.
\begin{algorithm}
...
\end{split}
\end{equation*}
where we again used FubiniÕs theorem for non negative functions.
\end{proof}
