...
\[\int\limits_{\mathcal X} K(x, A) \pi(\mathrm dx) = \frac1Z \delta_x(A) f(x) \mu(\mathrm dx) = \pi(A). \]
\end{proof}
...
So far we have presented the theoretical foundations of the MH random walk and now we want to touch on a few aspect of the simulation process. For this part we shall point the reader towards the very gentle introduction \cite{robert1999metropolis} to the implementation of the MH random walk which also provides coding examples. Further it shall be noted that we will not provide any rigorous results in this section but will give general statements about considerations one should make when implementing the MH random walk. In fact -- although we have seen that the empirical measures associated with the MH random walk converge to \(\pi\) under fairly mild assumptions -- it is mostly the choice of the proposal that determines the speed of this convergence.
...
where we used Markov’s inequality as well as \(y>0\) almost surely. Now draw \(x_{n+1}\) according to the uniform distribution on \(S\). Note that \(f(x_n)>0\), then \(f(x_{n+1})\ge y > 0\) almost surely.  In pseudo code for the algorithm can is presented in Algorithm \ref{alg:slice-sampling}.
\begin{algorithm}
