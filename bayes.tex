...
\begin{algorithm}
\caption{The MH random walk \label{alg:MH}}
\begin{algorithmic}[1]
...
\subsubsection*{Simulation of the MH random walk}
...
Let \(\Theta\) be a measurable space, \(\mu\) a measure on that space and \(f\colon\mathcal X\to[0, \infty]\) a function with finite integral
\[Z = \int f(x) \mu(\mathrm{d}x)\in(0, \infty).\]
In particular there is \(x_0\in\mathcal X\) such that \(f(x_0)>0\). Our goal is to find a Harris recurrent Markov chain with invariant distribution
\[\pi(A)\coloneqq \frac1Z \int_A f(x)\mu(\mathrm dx). \]
...
Assume we have already given the first \(n\) samples \(x_1, \dots, x_n\) of the Markov chain. If we have \(f(x_n)=0\), then we set \(x_{n+1}\coloneqq x_0\). Otherwise we sample \(y\) according to the uniform distribution on \([0, f(x_n)]\) and define the \emph{slice}
...
where we used MarkovÕs inequality as well as \(y>0\) almost surely. Now draw \(x_{n+1}\) according to the uniform distribution on \(S\). Note that \(f(x_n)>0\), then \(f(x_{n+1})\ge y > 0\) almost surely.

In pseudo code the algorithm can be written as
\begin{algorithm}
\caption{The slice sampling method \label{alg:slice-sampling}}
\begin{algorithmic}[1]
\Require{First \(n\) samples \(x_1, \dots, x_n\)}
 \If \(f(x_n) = 0\)
