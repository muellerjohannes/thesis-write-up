...
which we will call the \emph{prior} distribution of the parameter \(\theta\). Further let
\[\mathcal F = \Big\{ f_{X| \Theta}(\cdot |\theta) \mid \theta\in\Theta\Big\}\]
...
%The idea of the Metropolis-Hastings (MH) random walk was introduced as early as 1953 by Metropolis (c.f. \cite{metropolis1953equation}) and later adapted by Hastings in \{hastings1970monte}.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.42\textwidth]{alpha-small}
	\includegraphics[width=0.42\textwidth]{auto-alpha-small}
	\includegraphics[width=0.42\textwidth]{alpha-optimal}
	\includegraphics[width=0.42\textwidth]{auto-alpha-optimal}
	\includegraphics[width=0.42\textwidth]{alpha-big}
	\includegraphics[width=0.42\textwidth]{auto-alpha-big}
%	\tag{1}
...
\begin{emp}[Definition of the slice campling]
...
\begin{algorithm}
\caption{Sampling a uniform distribution on a subset \(S\subseteq C\) \label{alg:slice-sample}}
\begin{algorithmic}[1]
...
In conclusion this decision algorithm in combination with Algorithm \ref{alg:sample-slice} gives an expression of the slice sampling Markov chain completely through the bounds \(f_n^\pm\).
