...
\begin{equation*}
\begin{split}
\sum_{A\subseteq \mathcal Y, \left\lvert A \right\rvert = m} \det(K_A) & = \sum_{i_1, \dots, i_m\in\mathcal Y} \det\left(K_{i_ki_l}\right)_{1\le k, l \le m} \\
%& = \sum_{i_1, \dots, i_m\in\mathcal Y} \det(B^T_{i_kl})_{1\le k, l \le m} \det(B_{i_kl})_{1\le k, l \le m} \\
& = \sum_{i_1, \dots, i_m\in\mathcal Y} \det\left(B_{li_k}\right)_{1\le k, l \le m}^2 \\
& = \det\left( \sum_{i\in\mathcal Y} B_{ki}B_{li} \right)_{1\le k, l\le m} \\
& = \det\left( v_{k}^Tv_{l} \right)_{1\le k, l\le m} = 1
\end{split}
\end{equation*}
where we have used the Cauchy-Binet identity. It remains to check that all marginal probabilities satisfy
...
\begin{equation*}
\begin{split}
\mathbb P(A\subseteq \mathbf Y) & = \sum_{i_r\ne \dots\ne i_m\in\mathcal Y\setminus A} \mathbb P\big(A\cup\left\{ i_r, \dots, i_m\right\}\subseteq\mathbf Y\big) = \sum_{i_r\ne \dots\ne i_m\in\mathcal Y\setminus A} \det\left((v_l)_{i_k}\right)_{1\le k, l \le m}^2 \\
& = %\frac{(m-r)!}{m!}
\sum_{i_r, \dots, i_m\in\mathcal Y} \det\left((v_l)_{i_k}\right)_{1\le k, l \le m}^2 = \det\left( (v_{i_k})^T (v_{i_l}) \right)_{1\le k, l\le r-1} = \det(K_A) % \\\det\left(K_{i_ki_l}\right)_{1\le k, l\le m} \\
