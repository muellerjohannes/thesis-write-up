...
\end{equation}
where \(A\) is the matrix with the rows \(x(C_k)^T\). Now we can fix any such solution \(x = \phi(w)\in\mathbb F_2^{E}\) and we know that at least one exists, namely the one given by \(x_{ij} = \phi(\operatorname{sgn}(K_{ij}))\). %In the light of \eqref{e3.0} i
It is straight forward that \(\tilde K\) defined through
...
\begin{emp}[Definition of the estimator]
...
%\[\hat K_{ij}\coloneqq \begin{cases}  \hat w_{ij}\cdot \sqrt{(\hat B_{ij})} \quad & \text{if } \left\{ i, j\right\}\in \hat E \\ 0 & \text{if } \left\{ i, j\right\}\notin \hat E \end{cases}. \]
 %and otherwise
% \[\hat K_{ij}\coloneqq \sqrt{(\hat B_{ij})}.\]
\todo{state the result and see how hard it is to prove}
