...
The motivation for restriction our ambitions of estimations to the qualitis \(q_i\) rather than the whole elementary kernel \(L\in\mathbb R_{\text{sym}, +}^{N\times N}\) was to obtain a more tractable optimisation problem. In order to see whether we succeeded in that regard, we note that each summand in the log likelihood function takes the following form under the quality diversity parametrisation
...
\end{equation}
Unfortunately this still isnÕt concave in \(q\) and in order to achieve this, we will have to make following assumption.
