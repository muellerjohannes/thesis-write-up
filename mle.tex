...
\theta^T\sum\limits_{i\in Y}f_{i} + \det(\hat{S}_{Y}) - \log\left( \sum_{A\subseteq\mathcal Y} \exp\left( \theta^T\sum_{i\in A}f_{i}\right)\det(\hat{S}_A) \right).
\end{equation}
...
maximisers
\[\hat{\theta}_n \coloneqq \underset{\theta\in\Theta}{\arg\max} L_n(\theta).\]
If no maximiser exists, we choose \(\hat{\theta}_n\in\Theta\) arbitrary. Further let \(L\colon \Theta\to [-\infty, \infty)\) be a deterministic function with maximiser \(\theta_0\).
\end{emp}

The maximisers \(\hat{\theta}_n\) are called \emph{extremal} estimators since they are the extremal points of the functions \(L_n\). We now turn towards the question whether the extremal estimators converge to the maximiser \(\theta_0\).

\begin{theo}[Consistency of extremal estimators]
...
\item Let \(L_n\) converge to \(L\) locally uniformly in probability, i.e. for any compact set \(K\subseteq\Theta\) and any \(\varepsilon>0\) we have with probability tending to one
\begin{equation}\label{con1}%\mathbb P\Big( 
\big\lvert L_n(\theta) - L(\theta) \big\rvert \le \varepsilon \quad\text{for all }\theta\in K %\Big) \xlongrightarrow{n\to\infty} 1 \quad\text{for all } \varepsilon>0
.\end{equation}
%\item Let further 
...
%\mathbb P\Big( 
L_n(\theta) \le L(\theta_0) - \varepsilon_0 \quad \text{for all } \theta\notin K_0 %\Big) \xlongrightarrow{n\to\infty} 1
.\end{equation}
\item Let \(L\) have a unique maximum at \(\theta_0\in\Theta\).
\item Assume that \(L\) is %coercive and
...
%for some \(\delta>0\). This is due to the fact that
 \(L\) attains its maximum\footnote{Any upper semicontinuous function on a topological Hausdorff space attains its maximum over a compact set.}\todo{cite}{ } \(\alpha\) on \(K_0\setminus U\) which is strictly smaller than \(L(\theta_0)\). Thus we have %only have to show that the probability for \(\theta\in K\) and \(L(\hat{\theta})>\alpha\) tends to one since %. % \(\theta\in U\) and we will use this criterion to show the consistency.
 \[K_0\cap \Big\{ \theta\in\Theta\mid L(\theta) > \alpha \Big\} \subseteq U. \]
 
The probability for \eqref{con1} and \eqref{con2} tends to one so it suffices to show that they imply \(\hat{\theta}_n\in K_0\) as well as \(L(\hat{\theta}_n)>\alpha\). So lets assume \eqref{con1} with \(K=K_0\) and \eqref{con2} hold, then for \(\varepsilon<\varepsilon_0\) we obtain %we have %with probability tending to one
...
L_n(\theta) \le L(\theta_0) - \varepsilon_0 < L_n(\theta_0) \quad \text{for } \theta\notin K_0 \]
%where we used (\emph{i}) and (\emph{ii}). 
This implies that the maximum of \(L_n\) is attained in \(K_0\) and thus we have \(\hat{\theta}_n\in K_0\) and further
%\[\sup_{\theta\in K} \big\lvert L_n(\theta) - L(\theta) \big\rvert \le \varepsilon \]
% with probability tending to one. In this case we have
\[L(\hat{\theta}_n) \ge L_n(\hat{\theta}_n) - \varepsilon \ge L_n(\theta_0) - \varepsilon \ge L(\theta_0) - 2\varepsilon > \alpha\]
...
\[L_n(\theta) \coloneqq \frac1n\sum_{i=1}^n\log(f(X_i, \theta)).\]
...
L(\theta) \coloneqq \mathbb E\big[\log(f(X, \theta))\big]. \]
%in the case of maximum likelihood estimation.
 % where \(()\)
The quantity \(L\) is known as the \emph{relative entropy} or \emph{Kullback–Leibler divergence} and plays an important role in mathematical and applied statistics, information theory and many other fields. For more information on this we refer to\todo{find reference}.
...
\[\Big\{ f(\cdot, \theta)\colon X\to[0, \infty) \mid\theta \in\Theta\Big\}\]
be a family of probability densities on some measurable space \(X\) with respect to some measure \(\mu\). Further fix \(\theta_0\in\Theta\) and let \(\mathbb P\) denote the probability measure with \(\mu\) density \(f(\cdot, \theta_0)\) and denote the expectation with respect to \(\mathbb P\) by \(\mathbb E[\cdot]\).
%\(L_n \colon \Theta\to [-\infty, \infty)\)
...
\begin{lem}[Information inequality]
...
\[L(\theta) = \mathbb E\big[\log(f(X, \theta))\big] \]
