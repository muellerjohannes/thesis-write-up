...
% \\ & = \sum\limits_{i = 1}^n \log\left( \prod_{j\in Y_i} q_j^2\right) \cdot \det(\hat S_{Y_i}) - n
%\end{equation}
Unfortunately we can tell from \eqref{loglikqua} that the log likelihood still isnÕt concave in \(q\) and in order to achieve this, we will have to make following assumption and keep them in throughout this section.\todo{does it makes sense?}
...
%in the case of maximum likelihood estimation.
 % where \(()\)
The quantity \(F\) is known as the \emph{entropy} and plays an important role in mathematical and applied statistics, information theory and many other fields. For more information on this we refer to\todo{find reference}.


\subsubsection*{The information inequality and locally uniform convergence}

The third requirement of the previous consistency result can be proven in a general setting and without quantitative assumption and we adapt an argument from \cite{newey1994large} to fit our needs. In order to do this we will work with the following assumptions.
...
\end{equation}
In this case we want to exploit the strict Jensen inequality (c.f. \cite{lehmann2006theory}) that yields for any positive random variable \(Y\) with finite expectation that is not constant
...
Let the setting be as above, but let \(\Theta\) be a metric space and we assume that the following conditions hold.
\begin{enumerate}
\item Let \(K\subseteq\Theta\) be such that
...
\end{lem}
\begin{proof}
Fix \(\varepsilon>0\) and a compact set \(K\subseteq\Theta\). Define for \(\rho>0\)
...
.\]
For exactly those \(A\) we define now
\[K_A \coloneqq \Big\{ \theta\in\Theta \;\big\lvert\; \log(f(A, \theta)) \ge - c(A) \Big\}\]
which is compact\todo{check this}{ } since \(f(A, \theta)\) is upper semicontinuous and coercive in \(\theta\) with the same argument as in the coercivity proofs earlier in this chapter. Further it contains \(\theta_0\) as
\[\log(f(A, \theta_0)) \ge 2 \cdot \log(f(A, \theta_0)) > -c(A) \]
because \(f(A, \theta)\le 1\). Set now
\[K_0\coloneqq \bigcap_{A\in \mathcal A} K_A.\]
Finally we have for \(\theta\notin K_0\)
\begin{equation*}
\begin{split}
F_n(\theta) & = \int \log(f(x, \theta)) \hat{\mathbb P}_n(\mathrm{d} x) = \sum_{A\in \mathcal A} \hat{\mathbb P}_n(A)\cdot \log(f(A, \theta)) < - \sum_{A\in \mathcal A} \frac{f(A, \theta_0)}{2} c(A) \\
...
Obviously we want to exploit the machinery we have introduced and thus we will check the requirements of Theorem \ref{conext}. First we express the entropy function
...
Since the previous lemma already takes care of condition (\emph{i}) it suffices to show the second condition for which we will use \ref{locunicon}. Hence it remains to check the two conditions of this lemma, but the second one -- the continuity condition -- obviously holds as can be seen from \eqref{elepro}. To see that the first one also holds we note that for \(A\subseteq\mathcal Y\) with \(f(A, \theta_0)>0\) and \(\theta\in K_0\) we have
\[0 \ge \log(f(A, \theta)) \ge -c(A) > -\infty.\]
Hence the random variable
\[\sup_{\theta\in K_0} \big\lvert \log(f(X, \theta)) \big\rvert\]
is almost surely finite and since the probability space \(2^{\mathcal Y}\) is finite, the first condition holds.
\end{proof}
