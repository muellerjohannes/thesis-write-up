...
\todo{rewrite introduction to MLE}{ }
...
In fact every (upper semi-) continuous and coercive function defined on a closed set \(U\subseteq\mathbb R^M\) attains its minimum. To see this one can fix \(x_0\in U\) and use the coercivity to obtain \(f<f(x_0)\) outside of a compact set \(K\) and thus the supremum of \(f\) agrees with the supremum of \(f\) over \(K\cap U\) which is compact again and thus it is attained.
...
where \(f(A, q)\propto \det(\Psi(q, \hat S)_A)\) is the elementary probability of DPP with elementary kernel \(\Psi(q, \hat S)\). 
...
We notice that the single summands take the form following form\todo{refer to the equation!}
...
where we regard \(\mathcal L\) again as function of \(\theta\).
\end{emp}
...
\end{prop}
\begin{proof}
The first inequality is obvious since the log likelihood function is continuous.\todo{add remark}{ } We will show that \(\mathcal L\) is coercive if one of the observations is the emptyset. Then the claim follows from
\begin{equation*}
\begin{split}
...
\end{equation*}
Hence \(\theta_0\) is a critical point and under mild conditions the left hand side is concave and thus \(\theta_0\) is the unique maximiser. In conclusion the estimator \(\hat \theta\) should be close to \(\theta_0\).
\end{emp}

Although rough structure of the rigorous proof is present in the argument above it is highly formal. For example we argue that if a sequence \((f_n)_{n\in\mathbb N}\) of functions converges towards \(f\) pointwise, then the maximisers \((x_n)_{n\in\mathbb N}\) should converge to the maximiser \(x\) of \(f\). The major tool to make this rigorous will be to use some kind of uniform convergence. Namely we have the following result where we will omit the proof since it is very easy and we proof a similar but stronger version of it in the next section.

\begin{lem}[Swapping limit and maximisation]
Let \((f_n)_{n\in\mathbb N}\) be a sequence of real functions with maximisers \((x_n)_{n\in\mathbb N}\) that are bounded from above and converge uniformly towards \(f\). Further assume that \(f\) is coercive and continuous and let \(x\) be the unique maximiser of \(f\). Then we have \(x_n\to x\) for \(n\to\infty\).
\end{lem}
%\begin{proof}
...
We will prove a general consistency result for extremal estimators and first present the setting we will work in.
...
If no maximiser exists, we choose \(\hat{\theta}_n\in\Theta\) arbitrary. Further let \(F\colon \Theta\to [-\infty, \infty)\) be a deterministic function with maximiser \(\theta_0\).
\end{emp}

The maximisers \(\hat{\theta}_n\) are called \emph{extremal} estimators since they are the extremal points of the functions \(F_n\). We now turn towards the question whether the extremal estimators converge to the maximiser \(\theta_0\).
...
\item Let \(F_n\) converge to \(F\) locally uniformly in probability, i.e. for any compact set \(K\subseteq\Theta\) and any \(\varepsilon>0\) we have with probability tending to one
\begin{equation}\label{con1}%\mathbb P\Big( 
\big\lvert F_n(\theta) - F(\theta) \big\rvert \le \varepsilon \quad\text{for all }\theta\in K %\Big) \xlongrightarrow{n\to\infty} 1 \quad\text{for all } \varepsilon>0
.\end{equation}
%\item Let further 
...
\item Assume that there is \(\varepsilon_0>0\) and a compact set \(K_0\), such that with probability tending to one
...
\[\left\{ \theta\in\Theta \mid L(\theta)\ge\alpha\right\}\subseteq\Theta\]
...
\end{theo}
\begin{proof}
Fix an open set \(U\subseteq\Theta\) that contains \(\theta_0\). First we note that
...
%for some \(\delta>0\). This is due to the fact that
 \(F\) attains its maximum\footnote{Any upper semicontinuous function on a topological Hausdorff space attains its maximum over a compact set.}\todo{cite}{ } \(\alpha\) on \(K_0\setminus U\) which is strictly smaller than \(F(\theta_0)\). Thus we have %only have to show that the probability for \(\theta\in K\) and \(L(\hat{\theta})>\alpha\) tends to one since %. % \(\theta\in U\) and we will use this criterion to show the consistency.
 \[K_0\cap \Big\{ \theta\in\Theta\mid F(\theta) > \alpha \Big\} \subseteq U. \]
 
The probability for \eqref{con1} and \eqref{con2} tends to one so it suffices to show that they imply \(\hat{\theta}_n\in K_0\) as well as \(F(\hat{\theta}_n)>\alpha\). So lets assume \eqref{con1} with \(K=K_0\) and \eqref{con2} hold, then for \(\varepsilon<\varepsilon_0\) we obtain %we have %with probability tending to one
...
Note that the factor \(\frac1n\) does not change the maximum. However \eqref{entropy} already gives the almost surely pointwise limit of those functions and if condition (\emph{i}) of the previous statement should hold, we have to define
...
%in the case of maximum likelihood estimation.
 % where \(()\)
The quantity \(F\) is known as the \emph{relative entropy} or \emph{Kullback–Leibler divergence} and plays an important role in mathematical and applied statistics, information theory and many other fields. For more information on this we refer to\todo{find reference}.


\subsubsection*{The information inequality and locally uniform convergence}

The third requirement of the previous consistency result can be proven in a general setting and without quantitative assumption and is known as the information inequality. In order to d this we will work with the following assumptions.

\begin{emp}[Setting]
Let in the following \(\Theta\) be a set and let 
\[\mathcal F = \Big\{ f(\cdot, \theta)\colon X\to[0, \infty) \mid\theta \in\Theta\Big\}\]
be a family of probability densities on some measurable space \(X\) with respect to some measure \(\mu\). Further fix \(\theta_0\in\Theta\) and %let \(\mathbb P\) denote the probability measure with \(\mu\) density \(f(\cdot, \theta_0)\) and denote the expectation with respect to \(\mathbb P\) 
denote the expectation with respect to \(f(\cdot, \theta_0) \mathrm{d}\mu\) by \(\mathbb E[\cdot]\). Let \((X_n)\) be a sequence of independent random variables distributed according to \(f(\cdot, \theta_0) \mathrm{d}\mu\). Finally define
...
\begin{prop}[Information inequality]
...
for all \(\theta\in\Theta\). Then the relative entropy
\[F(\theta) = \mathbb E\big[\log(f(X, \theta))\big] \]
has a unique minimum in \(\theta_0\).
\end{prop}
\begin{proof}
Jensen’s inequality.
\end{proof}

Next we take care of the first requirement of the consistency result. Namely we will show that the functions \(F_n\) associated with the MLE almost surely converge to \(F\) locally uniformly under fairly mild conditions. For this we modify the proof of a more general convergence result in \cite{tauchen1985diagnostic}.

\begin{lem}[Locally uniform convergence]
Assume that the following conditions hold.
\begin{enumerate}
\item For any compact \(K\subseteq\Theta\) we have
\[\mathbb E\left[\sup_{\theta\in\Theta} \big\lvert \log(f(X, \theta)) \big\rvert \right]<\infty.\] 
\item For every \(\theta\in\Theta\) we have \(\log(f(\cdot, \gamma))\to\log(f(\cdot, \theta))\) almost surely with respect to \(\mathbb P\) for \(\gamma\to\theta\).
\end{enumerate}
%Then we almost surely have 
Then we almost surely have \(F_n\to F\) locally uniformly, i.e. almost surely we have for any compact \(K\subseteq\Theta\)
...
In this part we will -- for the first time -- make use of the specific structure the elementary probabilities. Since we have already taken care of condition (\emph{i}) and (\emph{iii}) and condition (\emph{iv}) is fairly straight forward, we first dedicate ourselves to proving the second requirement. For this we keep the setting of the previous section although we now consider the case that
\[\mathcal F = \Big\{ f(\cdot, \theta)\colon X\to[0, \infty) \mid\theta \in\Theta\Big\}\]
 is one of the parametric families introduced in \ref{models}.% Further we assume that the data \((\mathbf Y_n)\) are indepentend and distributed according to a \(L\)-ensemble with elementary probabilities \(f(\cdot)\).\todo{formulate this better!}

\begin{lem}[Control outside of a compact set]
The requirement \emph{(}ii\emph{)} from Theorem \ref{conext} is satisfied for the three kinds of parametric families for the kernel estimation.
\end{lem}
\begin{proof}
Set \(\delta\coloneqq\frac{f(\varnothing, \theta_0)}{2}>0\) and let 
\[\hat{\mathbb P}_n\coloneqq \frac1n \sum_{i=1}^n \delta_{\mathbf Y_i} \]
 be the empirical measure. Then we have by the law of large numbers
\[\mathbb P\left( \hat{\mathbb P}_n(\varnothing) \ge \delta\right)\xlongrightarrow{n\to\infty} 1.\]
Note that we have
\[F_n(\theta) = \int \log(f(x, \theta)) \hat{\mathbb P}_n(\mathrm{d} x) \le \hat{\mathbb P}_n(\varnothing)\cdot \log(f(\varnothing, \theta)) \]
since the elementary probabilities \(f(\cdot, \theta)\) are bounded by \(1\). Let now \(c>0\) be such that
\[-\delta c< F(\theta_0).\]
Choose now \(K\subseteq\Theta\) compact such that\todo{say way this is possible}
...
\[\mathbb P\left( \big\lVert \hat L_n - L_0 \big\rVert \le \varepsilon\right) \xlongrightarrow{n\to\infty} 1 \quad \text{for all }\varepsilon>0.\]
...
\[\mathbb P\left( \left\lVert \hat q_n - q_0 \right\rVert \le \varepsilon\right) \xlongrightarrow{n\to\infty} 1\quad \text{for all }\varepsilon>0.\]
\item Suppose that the observations \((\mathbf Y_n)\) follow the law of a \(L\)-ensemble with kernel \(\Psi(p_0, \hat S)\) where \((p_0)_i = \exp(\theta^Tf_i)\) and let \(P\) denote the projection onto the subspace \(U\). Then we have
\[\mathbb P\left( \big\lVert P\hat \theta_n - P\theta_0 \big\rVert \le \varepsilon\right) \xlongrightarrow{n\to\infty} 1\quad \text{for all }\varepsilon>0.\]
\end{enumerate}
\end{theo}
\begin{proof}
•
